...
| 0.8.x      | >=0.14.0     | >=0.11.0       | >=0.7.0        | >=0.5.0             | >=0.5.0               | >=0.4.0               | >=0.1.0          | >=0.1.0,<0.2.0     | >=0.2.0               |
...
**Shipped on PyPI as of 2026-09-10 — but only the data-client floor guarantees it.** Verified against the *published* wheels in a clean venv, not a checkout: `juniper-data-client` **0.5.0** and `juniper-cascor-client` **0.8.0** both expose their `Juniper*ConfigurationError`, both refuse a hostless `https://`, and both NORMALISE `HTTPS://host` to `https://host` rather than talking plaintext — so the TLS-downgrade reading of this note is withdrawn. `[clients]` now pins `juniper-data-client>=0.5.0`, which does guarantee the guard. It still pins `juniper-cascor-client>=0.5.0`, and **that floor does not**: 0.8.0 has the guard, but the resolver may legally pick an older wheel. Raising that floor is an open decision, unrelated to decision 11. `juniper-recurrence-client` is not in `[clients]` at all — it ships under `[recurrence]`.
...
| `HTTPS://host` talks HTTP / hostname is `https` | Wheel predates the case-insensitive scheme check. **Fixed in the published wheels**: data-client `0.5.0` and cascor-client `0.8.0` normalise the scheme (verified 2026-09-10 against PyPI, not a checkout). Only reachable now via a cascor-client older than 0.8.0, which `[clients]`' `>=0.5.0` floor still admits. |
