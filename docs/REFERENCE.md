...
The stopping rule in `soak_run_probe.py` rations **billed sessions**. It has **two** arms, and they refuse for opposite reasons: `verdict_is_terminal()` (`BET-FAILING` / `HOLDS-AT-` **prefix**) — the soak has an answer further runs cannot change — and `verdict_is_unusable()` (**exact** membership of `""`, `DEGRADED`, `NO-DATA`, `NO-SEEDED-DATA`) — the ledger cannot say what state the soak is in. `refuses_terminal_verdict(...)` / `refuses_unusable_verdict(verdict, force=, dry_run=)` are both false when `force` **or** `dry_run` is set (juniper-ml#1690, extended 2026-09-10) — the exemptions are not symmetric: `--force` is a deliberate spend override; `--dry-run` spends nothing and was never in the rule's scope.

The second arm was added 2026-09-10; before it, **`DEGRADED` / `NO-DATA` / `NO-SEEDED-DATA` and a crashed ledger tool all passed the spend control and dispatched a billed session against a corpus nobody could read.** `st.returncode` is still never consulted, and that is deliberate — re-measured 2026-09-10, `soak_ledger.py status` exits **1** for a real crash *and* for a perfectly runnable `INCONCLUSIVE` with an open escalation, and **2** for `NO-DATA` *and* for argparse misuse. The exit code cannot separate those; the verdict token can.

| Invocation | Terminal (`BET-FAILING` / `HOLDS-AT-*`) | Unreadable (`DEGRADED`, `NO-DATA`, `NO-SEEDED-DATA`, crash) | Runnable (`INCONCLUSIVE`, `IN-PROGRESS`) |
|------------|------------------------------------------|--------------------------------------------------------------|------------------------------------------|
...
`soak_run_probe.py` exit codes: **0** ran · **1** ran but no usable answer (timeout / error) · **2** argparse misuse or harness failure before the probe started · **3** refused by design.
...
- `tests/test_soak_run_probe_stopping_rule.py` -- Stopping-rule paths in `util/soak_run_probe.py` that the dry-run suite cannot reach: the dry run never enters the arms that decide to STOP, so those arms shipped unexercised. Also the **fail-open** closure (2026-09-10): an unreadable ledger refuses, the refusal exit code is pinned as a real subprocess status, every verdict `soak_ledger.py` can emit is read from its AST and must be classified, and the systemd unit is parsed so its `SuccessExitStatus=` cannot drift from the guard.
...
- `tests/test_soak_run_probe_terminal.py` -- The terminal-verdict stopping rule as a pure parse: only the FIRST token of `soak_ledger status` is the verdict, so a later mention of a terminal name in the counts must not trip the guard -- and `--dry-run` is EXEMPT, because the rule rations billed sessions and a dry run spends none.
...
- `tests/test_pf8_icv_checkpoint_probe.py` -- The PF-8 ICV pair (`util/ad-hoc/2026-09-11_omp_icv_checkpoint_probe.py`, `2026-09-11_icv_trace_align.py`), which overturned two more published conclusions: the attribution note's named suspect for what ends the burst -- candidate-pool creation -- does **not** re-pin (`_ensure_worker_pool` returns with the ICV still 16), and its §5.1 reading of the matmul is inverted (the sustained matmul re-pins its own thread and then runs at that width, rather than being immune to which thread it runs on). Pins that the checkpoint does **not** call `torch.get_num_threads()` by default, because that getter is not a passive read -- it runs torch's per-thread lazy init and re-pins the calling thread, so a checkpoint that read it would end the burst it exists to observe (the probe's own first draft did exactly this, and every arm including the control reported an identical instrument-caused re-pin); that the icv-map keeps a do-nothing **control** arm and prints a loud failure line when it re-pins, since an instrument footprint is otherwise indistinguishable from a result; that every arm runs on its own fresh thread, because a re-pinned thread stays re-pinned and a shared thread would let the first arm mask every later one; that the `output_pass` arm does not construct its network on the thread being measured, which would measure the constructor pin instead of the pass; that the reducer scores "the drop **ends** the burst" (refuted -- the burst was over ~2.9 s earlier) and "the drop is why **later** passes do not burst" (supported) as separate claims and **computes** both rather than hard-coding them; that growth mode reports which stages it did not wrap, so an absent checkpoint is never readable as "this stage did not re-pin"; and that the spiral stays at `radius_scale` 10.0, since a unit-radius spiral grows zero hidden units and the growth loop under test would never run.
...
- `util/safe_merge.py` -- Merge gate (synthesis R4): merges a PR **only** after its REQUIRED checks have finished green, refusing otherwise. Closes the measured failure where **12% of freshness-synced PRs merged before the re-test they had just paid for could finish**.
  ...
  - **CI budget is PER-REPO** (`REPO_TIMEOUTS`, `timeout_for()`), across *all required contexts on one head*, not one workflow. The prior single `900 s` was derived from ml's `ci.yml` median alone and sat at **canopy's median**, so roughly half of canopy's healthy merges would have refused with "checks did not finish".
    **Re-measured fleet-wide 2026-09-08, n=30, required contexts only** (`util/ad-hoc/2026-09-08_measure_required_check_span_v2.py`): ml p90 538 s / max 773; data 679 / 1047; cascor 770 / 2561; cascor-worker 888 / 1029; canopy 1597 / 2067; cascor-client 724 / 1511; data-client 893 / 1109; deploy 262 / 375; recurrence 258 / 352. Budgets: ml 1500, data/worker/data-client 2400, cascor 2800, canopy/cascor-client 3300, deploy/recurrence 700; `DEFAULT_TIMEOUT` 2400 remains the fallback for a repo not in the table.
    **The 2026-08-20 figures this row used to carry came from a broken instrument.** `util/ad-hoc/2026-08-20_measure_required_check_span.py` filters nothing despite its name, so advisory and bot check-runs on the head SHA enter the span — it overstated the observed max by 11x–110x, and made the p90 swing ~3x on sample size alone. Two claims it produced are **withdrawn**: that cascor-client's 15,616 s max was *queue time* (it is bot check-runs; the real max is 1511 s), and that sizing must therefore use **p90 and never max**. Both statistics now bind — `> observed max` and `<= 4x p90` — and `tests/test_safe_merge.py::test_default_timeout_is_sized_from_measurement` asserts both. Under the p90-only form, cascor's 2400 s budget sat below its own 2561 s observed max and the test still passed.
    `TIMEOUT_CEILING = 3300` sits below the 3600 s worker lease, because a local wait longer than the lease is unreachable for a background-run invocation.
...
**Consumer-doc stragglers — CLOSED** (re-verified against each sibling's `main`, 2026-09-09). An earlier revision of this section listed four consumer documents as still publishing the old key list. Three did and are fixed: `juniper-recurrence-client/README.md` (juniper-recurrence#152, `46f3faf` — it now says the whole view uses a producer-supplied `*_full` when present and is otherwise rebuilt from `train | val | test`), `juniper-canopy/docs/demo/DEMO_MODE_REFERENCE.md` (juniper-canopy#604, `e09e6c8`), and `juniper-data-client/docs/REFERENCE.md` (juniper-data-client#193, `5f4d48c`). The fourth, `juniper-recurrence/README.md`, was listed **in error** — `git log -S'_full' -- README.md` on that repo is empty, so it never carried an `X_full` key; its one occurrence of the word is `RECURRENCE_SPLITS`' deliberately-retained `"full"` value, which is correct. Transcribing a straggler list instead of probing each file is how the wrong entry got in, which is the same failure this section's preamble warns about.
...
- **The PUBLISHED `juniper-model-core` 0.3.1 wheel carries a docstring asserting a dead contract** — the repo does not. `crossval/splits.py` was corrected on `main` by juniper-ml#1829 (`5e41f67a`) and now states that the producer no longer emits a `*_full` family and the caller rebuilds the whole dataset from `train` / `val` / `test`, naming `juniper_recurrence_model.data.derive_full_split` as the reference implementation. But the fix landed *after* 0.3.1 shipped and carried no version bump, so repo and PyPI both read `0.3.1` while their contents differ, and `pip install juniper-model-core` still serves the old text: *"folds are derived client-side from the `*_full` arrays; no juniper-data change is required for v1."* Behaviour was never affected (juniper-recurrence#150). A 0.3.2 patch release is what closes this; `detect.py` offers it.
