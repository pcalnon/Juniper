...
| Utility | Purpose | Key Overrides |
|---------|---------|---------------|
| `util/isolated_stack.bash --up` | Create the data venv, then launch data → cascor → canopy (health-gated); a mid-leg failure tears the partial trio back down. Both service legs stamp `JUNIPER_{CASCOR,CANOPY}_GIT_SHA` / `_BUILD_DATE` from the checkout they launch, reported on `/v1/health` as `git_sha` (2026-09-08) | `JUNIPER_E2E_DATA_PORT`, `JUNIPER_E2E_CASCOR_PORT`, `JUNIPER_E2E_CANOPY_PORT`, `JUNIPER_E2E_HEALTH_TIMEOUT`, `JUNIPER_E2E_DATA_EXTRAS`, `JUNIPER_E2E_RUN_DIR`, `JUNIPER_E2E_CASCOR_WS_EXTRA_ORIGINS` (extra canopy origins for cascor's control-WS allowlist, CSV — a second canopy beside the trio, e.g. the `:8052` verify leg, 403-loops without it), `JUNIPER_E2E_*_CONDA` / `*_DIR` |
