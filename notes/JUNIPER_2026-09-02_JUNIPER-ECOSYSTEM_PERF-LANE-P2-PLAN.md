...
- **The experiment YAML's `runtime:` block binds nothing.** `runtime.blas_threads` /
  ...
  with the listener's OpenBLAS pool at the host's full width; pinning
  `OMP/MKL/OPENBLAS_NUM_THREADS=2` + `CASCOR_NUM_PROCESSES=4` in the *process env* is 34% faster per
  step at an identical `step_count`. Fix the budget explicitly in every new scenario; wiring the
  block into the launcher is an owner decision, because it changes what every existing YAML means
