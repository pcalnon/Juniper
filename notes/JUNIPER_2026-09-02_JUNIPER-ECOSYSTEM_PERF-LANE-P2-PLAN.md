...
| #   | Item | Repo | Size | Depends on |
|-----|------|------|------|------------|
| 2.1 | **PARTIALLY DISCHARGED 2026-09-05 — calibration state established, run deferred with 2.2.** Resolved against its base config: `max_wall_seconds: 600` (inherited) < `per_run_timeout_seconds: 2400`, so **the driver budget binds first — §4 ordering is SATISFIED**; and `max_epochs`/`output_epochs` are **both** 50 post-`cascor#618`, so **there is no epoch-split hazard**. Two of the three concerns in the 2026-09-04 handoff's item 6 are therefore discharged. What stands: the native `(2, 2)` / 50-epoch budget runs **15.09 s**, short of PF-1's ~60 s. **No epoch override is proposed** — PF-1 reached 4000 by probing 500/2000/5000, and PF-2 is harder (an **8×** dataset range — `n_points_per_spiral` 250 → 2000; this row and the G-17 note said "10×" until 2026-09-08 — must clear the floor at its smallest cell and stay inside 600 s at its largest). A guessed number would look like calibration without being it. Detail: [`JUNIPER_2026-09-05_JUNIPER-ECOSYSTEM_PERF-LANE-G17-BRIDGE-EVIDENCE-AND-HOST-WINDOW.md` §5](JUNIPER_2026-09-05_JUNIPER-ECOSYSTEM_PERF-LANE-G17-BRIDGE-EVIDENCE-AND-HOST-WINDOW.md). **PROBED 2026-09-09 — and the probe re-scopes the item.** `util/ad-hoc/2026-09-08_pf2_epoch_calibration_suite.yaml`: both dataset endpoints × 1000 / 2000 / 4000 epochs (matched pairs, `include` cells) plus the bare base-config cell, seven cells in 5.5 min. **The dataset axis is inert**: `step_count` is identical at 250 and 2000 points at every budget (450 / 890 / 1770 — and 1770 is PF-1's invariant), and `step_sum` at 2000 points is 6–8% *lower*, inside the noise band. One value clears both ends trivially — 4000/4000 gives 45–50 s `drive` at either end, above the scrapeability floor and nowhere near the 600 s wall — but **PF-2 as written would measure an invariant**. Whether to widen the range by orders of magnitude or retarget the scenario at the candidate phase is an **owner call** on a §12.3 definition; the suite file is unchanged. §4–§5 of [`JUNIPER_2026-09-08_JUNIPER-ECOSYSTEM_PERF-LANE-PF8-RESCOPE-AND-MICRO-TIMING-REFERENCE.md`](JUNIPER_2026-09-08_JUNIPER-ECOSYSTEM_PERF-LANE-PF8-RESCOPE-AND-MICRO-TIMING-REFERENCE.md). Execute **PF-2** (cascor dataset-size scaling) and file evidence | juniper-ml | S | 0.3 |
| 2.2 | **BLOCKED 2026-09-10 — the suite's second matrix axis is INERT.** `pf3-cascor-pool-scaling.yaml` varies `runtime.num_processes: [1, 2, 4]`, and the experiment YAML's `runtime:` block is read by nothing on either path (§4 of [`JUNIPER_2026-09-10_JUNIPER-ECOSYSTEM_PERF-LANE-PF8-OCCUPANCY-PROBE.md`](JUNIPER_2026-09-10_JUNIPER-ECOSYSTEM_PERF-LANE-PF8-OCCUPANCY-PROBE.md)), so as written the approved ~6.7 h run is four pool sizes run three times each and cannot produce its speedup curve. Do not launch it until the `runtime:` block is ruled (owner: implement → `CASCOR_NUM_PROCESSES` exported per cell from `runtime.num_processes`; retire → re-express the axis). Earlier: **DEFERRED 2026-09-05 — approval GIVEN, window NOT available.** Host measured at load avg 14.75 on 16 cores, 6.0% iowait, swap 20997/20999 MB. §8.4's own table puts that past the 12-worker point measured at **+181.6%**, so the result would be dominated by ambient load rather than by pool × processes. Reasoning: [`JUNIPER_2026-09-05_JUNIPER-ECOSYSTEM_PERF-LANE-G17-BRIDGE-EVIDENCE-AND-HOST-WINDOW.md` §4](JUNIPER_2026-09-05_JUNIPER-ECOSYSTEM_PERF-LANE-G17-BRIDGE-EVIDENCE-AND-HOST-WINDOW.md). Nothing else blocks it. Execute **PF-3** (candidate-pool × process scaling). Largest host-time cost in the lane: a 4×3 matrix at 2000 s driver budget. Needs an explicit host-time approval and a quiet window. | juniper-ml | M | 0.3 |
...
| 2.4 | **DONE 2026-09-09 — `juniper-cascor#638`.** The reference exists: pytest-benchmark's saved run, cut with `--benchmark-autosave` into `~/.local/state/juniper-experiments/baselines/cascor-micro` (outside every checkout — `git worktree remove` deletes ignored files), host identity stamped into `machine_info` (compared, warns on change) and the load average at save time recorded beside it (never compared). Two cuts, `0001` / `0002`, both on a loaded host (1-minute load 9–10 at save) and therefore **provisional** until a quiet re-cut. Report-only per 2.5: `--benchmark-compare=NNNN`, never `--benchmark-compare-fail`. Census before claiming "never": all **22** `baseline_*.json` files that ever existed (21 tracked until cascor `971d35a` deleted them, plus the live one), 312 entries, zero timing keys. §2 of [`JUNIPER_2026-09-08_JUNIPER-ECOSYSTEM_PERF-LANE-PF8-RESCOPE-AND-MICRO-TIMING-REFERENCE.md`](JUNIPER_2026-09-08_JUNIPER-ECOSYSTEM_PERF-LANE-PF8-RESCOPE-AND-MICRO-TIMING-REFERENCE.md). Original: **PF-4 — establish** a cascor micro-level *timing* baseline. `baseline_20260526.json` holds 10 entries with **zero** timing data, and `test_baselines.py` defines three memory tolerances and no *baseline-relative* timing tolerance (its two `*_TIME_THRESHOLD_S` asserts are fixed absolute ceilings — the row said "no timing tolerance" until 2026-09-08). PF-4's first task is creating the reference, not comparing against one. | juniper-cascor | M | 0.1 |
...
| 3.3 | **DONE 2026-09-05 — PASSES.** Run `20260905T200957Z-9d59` (`pf-g17-recurrence-bridge`, driver exit 0) put `juniper_recurrence_train_last_duration_seconds` = 0.0783 and `crossval_last_duration_seconds` = 0.0961 into Prometheus under `environment="host-experiment"` — 144 series / 34 metric names, from zero before the run. **The plumbing was correct as believed; the item was unexecuted, not broken.** Evidence + two operator traps (worktree-resolved targets dir fails SILENTLY and inverts the result; `/metrics` 307-redirects so a naive scrape reads empty): [`JUNIPER_2026-09-05_JUNIPER-ECOSYSTEM_PERF-LANE-G17-BRIDGE-EVIDENCE-AND-HOST-WINDOW.md`](JUNIPER_2026-09-05_JUNIPER-ECOSYSTEM_PERF-LANE-G17-BRIDGE-EVIDENCE-AND-HOST-WINDOW.md). **G-17 second sub-item**: launch a recurrence run with `--grafana-bridge` and confirm recurrence timings actually appear under `environment="host-experiment"`. The enabler shipped in `juniper-ml#1547`; the consumer item was dropped. | juniper-ml | S | 3.1 |
...
> **Correction (2026-09-08, item 4.3).** Both halves of that premise were stale when this plan was
> written. Two identical cells of **one** suite *are* two simultaneous runs — PF-8 never needed two
> suites — and the "cascor `parallel > 1` still refused" constraint quoted in 4.1 was lifted on
> 2026-08-30 by a fail-closed version floor (`CASCOR_PARALLEL_FLOOR = (0, 10, 0)`, read from the
> tree that will launch; the primary is 0.11.0). A two-cell parallel cascor suite dry-runs to exit
> 0 with disjoint ports and pinned equal thread budgets, all provided by existing code. §1 of
...
| #   | Item | Repo | Size | Depends on |
|-----|------|------|------|------------|
| 4.1 | **DONE 2026-09-10 — the occupancy probe ran, twice** (§2 of [`JUNIPER_2026-09-10_JUNIPER-ECOSYSTEM_PERF-LANE-PF8-OCCUPANCY-PROBE.md`](JUNIPER_2026-09-10_JUNIPER-ECOSYSTEM_PERF-LANE-PF8-OCCUPANCY-PROBE.md)): one PF-1-shape run consumes **4.52 cores** (cpu-seconds per wall-second; the sweep's worker unit is an assumption §8.4 of the sweep note disclaims) under cascor's default thread budget — 3 cells, 1.1% spread, as **one ~11-core block for the initial output pass** (160 of 1770 steps at 92–130 ms, before the candidate pool exists) then ~1.8 for the other ten passes — and **2.15** under the four-variable budget `thread_budget_env` pins for a parallel arm. Pinned is 34% faster per step overall — **8× on the initial pass, +9.3 to +10.4% on the other 91% of steps (split-dependent)** — at an identical `step_count` 1770. The default figure sits inside the ~4–8 band, so step 2 ran (4.2). The burst is removed by the three BLAS variables, which the YAML's `runtime.blas_threads` never sets; which library carries it (NumPy's OpenBLAS pool is the leading candidate; unpinned torch at the tiny op shape burns 7.5 cores) is narrowed, not identified (§4.2 of that note). Earlier: **WITHDRAWN as a build item 2026-09-08 (item 4.3): the harness exists** — `run_suite` parallel mode allocates disjoint ports per cell (`experiment_stack.bash --up`, 8230–8259), pins equal thread budgets (`thread_budget_env`: `CASCOR_NUM_PROCESSES=4` + BLAS 2 at `max_parallel: 2`, recorded per cell), and collects both. What remains of 4.1 is an **S-sized occupancy probe**: sample the cascor process tree's CPU during one PF-1 cell and express it in sweep-worker equivalents, so a second run's cost can be read off §8.4's curve; a two-arm measurement only if that lands in the ~4–8 worker-equivalent band around the 6–8-worker knee (§1.3 of the re-scope note). Original text retained: Concurrent-launch harness: start two suites with pinned, equal thread budgets and disjoint port ranges, and collect both. Must not reuse the sweep driver's naive teardown — see §4 of this document. **Two constraints from `util/experiments/run_suite.py:18-20`**: cascor `parallel > 1` is still **refused from one checkout** (Q-6's override landed in `cascor#523`, but `run_suite` cannot verify the installed cascor honours it — H-7), so a two-cascor-run PF-8 needs either two checkouts or a verified override; recurrence is unconstrained. A cascor+recurrence pairing is the cheapest first arm. | juniper-ml | L | 1.4 |
| 4.2 | **EXECUTED 2026-09-10 from `util/ad-hoc/`** (§3 of [`JUNIPER_2026-09-10_JUNIPER-ECOSYSTEM_PERF-LANE-PF8-OCCUPANCY-PROBE.md`](JUNIPER_2026-09-10_JUNIPER-ECOSYSTEM_PERF-LANE-PF8-OCCUPANCY-PROBE.md)): `2026-09-10_pf8_two_run_parallel_suite.yaml` run three times by `2026-09-10_pf8_pair_driver.bash` (three aligned pairs — start offsets 0.003–0.068 s, ≥ 89% shared drive window) against six sequential controls under the same four variables, all twelve cells at `step_count` 1770 — **a second concurrent pinned run costs +11.3% per step (+8.5 to +12.7% leaving one of the three pairs out; three effective observations, not six)**, inside the sweep's 20.5% quiet band (the lane's standing rule: below what this host can measure across sessions) and outside the day's 4.7–8.7% within-arm spread by a margin one pair wide. Advisory, as 4.3 said. **The CI hazard below is therefore OPTIONAL**: a committed `suites/perf/` pair is needed only if PF-8 is to be re-run routinely — owner. Two *unpinned* runs were not measured (`run_suite` always pins in parallel mode). Earlier: **Re-specified 2026-09-08 (§1.4 of the re-scope note): a suite PAIR** — `pf8-two-run-concurrency-parallel.yaml` and `-control.yaml`, identical except for `execution`, the control run with the same four thread variables exported by hand (`run_suite` pins them only in parallel mode; a mismatched pair is refused by `compare_baseline`'s `thread_budget` identity, correctly). Instrument: mean step duration, never `drive`; quiet cells bracketing; n ≥ 3 per arm. **BLOCKED on a CI hazard (§1.5)**: `tests/test_experiment_suite_yamls.py` loads every checked-in suite in CI, where no cascor sibling exists, and the parallel floor check fails closed there — so a parallel cascor suite under `suites/perf/` turns the R-6 drift gate red on every CI run while passing locally. Recommended fix: move the floor check from `load_suite` to the execution path (still before any launch, still fail-closed). Original: `perf/pf8-two-run-concurrency.yaml` (or a harness-level descriptor if a suite cannot express it) + execution + evidence | juniper-ml | S+M | 4.3 |
| 4.3 | **DONE 2026-09-08.** PF-8 has **no gate content**: `step_count` was invariant across a 3× load span in all 21 sweep cells, so a second run cannot move the work gate, and speed is structurally ungated. Its speed half is characterised in synthetic-worker units but not **located** — the marginal value is finding where one Juniper run sits on §8.4's axis (an occupancy probe, S), and only if that lands near the knee does a two-arm measurement add information; inside the 20.5% band the answer is "below what this host can measure", not "free". The harness Wave 4 planned to build already exists (4.1). §1 of [`JUNIPER_2026-09-08_JUNIPER-ECOSYSTEM_PERF-LANE-PF8-RESCOPE-AND-MICRO-TIMING-REFERENCE.md`](JUNIPER_2026-09-08_JUNIPER-ECOSYSTEM_PERF-LANE-PF8-RESCOPE-AND-MICRO-TIMING-REFERENCE.md). Original: **Reconcile PF-8 with the headroom sweep.** §8.4 of the instrument-resolution results already answers a neighbouring question — the knee is between 6 and 8 competing workers — so PF-8's marginal value is now *"what does a second **Juniper** run cost"*, not *"is contention real"*. Re-scope before building 4.1, or it measures something already known. | juniper-ml | S | 1.4 |
...
0.1 ─┬─> 0.2 ──> 0.3 ─┬─> 1.1 ──> 1.2 ─┬─> 1.3
     │                │                ├─> 1.4 ──> 4.3 ──> 4.2        (4.1 withdrawn 2026-09-08: the harness exists; probe DONE and 4.2 EXECUTED 2026-09-10)
     │                │                └─> 1.5 (owner)
     ├─> 0.4 ─────────┘
...
- **`include` cells do not inherit `matrix`** — repeats must be a matrix axis, or the cells are not
  repeats of each other. The converse also holds: an `include`-only suite **still emits the bare
  base-config cell** (`c000`, `overrides={}`) — both epoch-calibration probes carried one.
- **The experiment YAML's `runtime:` block binds nothing.** `runtime.blas_threads` /
  `runtime.num_processes` are validated by the driver (`util/experiments/run_experiment.py:171`,
  `:607-609`), accepted by the cascor service as a known block and then ignored
  (`juniper-cascor/src/api/settings.py:142-144` projects only `service:`), never exported by the
  launcher, and never read by the direct CLI. Every service-path speed figure in this lane was taken
  with the listener's pools at the runtime default: its *initial output pass* runs ~8× slower than
  with `OMP/MKL/OPENBLAS_NUM_THREADS=2` + `CASCOR_NUM_PROCESSES=4` in the *process env* (34% per
  step overall, +9.3 to +10.4% on the other 91% of steps), at an identical `step_count`. **PF-3's
  `runtime.num_processes` axis is inert** (row 2.2). Fix the budget explicitly in every new
